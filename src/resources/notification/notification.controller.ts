...
import { AddFcmTokenOrPlayerIdDto, FetchNotificationsDto, } from "./notification.dto";
...
class NotificationController implements Controller {
  ...
  initializeRoute(): void {
    ...
    this.router.get(
      ...
      validationMiddleware(validate.fetchNotificationsSchema, RequestData.query),
  ...
  private markNotificationAsRead = async (
    ...
    try {
      ...
      const {
        ...
        message,
        data
      } = await this.notificationService.markNotificationAsRead(user, notification_id);
      return responseObject(
        res,
        code,
        status,
