...
import {
	GoogleUser,
	RequestExt,
} from "@/utils/interfaces/expRequest.interface";
...
class UserController implements Controller {
	...
	initializeRoute(): void {
		...
		this.router.get(
			...
			this.passportInst.authenticate("google", { scope: ["profile", "email"] })
		);
		...
		try {
			passport.authenticate(
				...
				async (err, user, info) => {
					...
					const { status, code, message, data } =
						await this.userService.googleAuthCallback(gUser);
	...
	private addShippingAddress = async (
		...
		try {
			...
			address.user = req._user || new userModel();
			const { status, code, message, data } =
	...
	private getProfile = async (
		...
		try {
			const user: User = req._user || new userModel();
			const { status, code, message, data } =
	...
	private getVendorDashboard = async (
		...
		try {
			...
			const timeFrame =
				is_custom === false &&
				req.query?.time_frame &&
				req.query?.time_frame !== null &&
				req.query?.time_frame !== ""
					? String(req.query?.time_frame)
					: undefined;
			const custom_date =
				is_custom === true
					? {
